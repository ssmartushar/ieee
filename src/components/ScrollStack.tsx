...
const ScrollStack: React.FC<ScrollStackProps> = ({
  ...
  const updateCardTransforms = useCallback(() => {
    ...
    const isMobile = scroller.clientWidth < 640;
    const effectiveBaseScale = isMobile ? Math.min(0.98, baseScale + 0.08) : baseScale;
    const effectiveItemScale = isMobile ? Math.max(0.02, itemScale - 0.01) : itemScale;
    const effectiveItemStackDistance = isMobile ? Math.max(20, itemStackDistance - 6) : itemStackDistance;
    const effectiveItemDistance = isMobile ? Math.max(80, itemDistance - 20) : itemDistance;
